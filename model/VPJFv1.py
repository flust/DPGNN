...
class VPJFv1(PJFModel):
    def __init__(self, config, pool):

        super(VPJFv1, self).__init__(config, pool)
        ...
        self.text_matching_mlp = MLPLayers(
            layers=[self.bert_embedding_size, self.hd_size, self.hd_size],
    ...
    def _text_matching_layer(self, interaction):
        ...
        text_matching_vec = self.text_matching_mlp(bert_vec)    # (B, H)
    ...
    def _intent_modeling_layer(self, interaction):
        ...
        job_desc_vec = torch.sum(job_desc_vec, dim=1) / \
                  job_longsent_len.unsqueeze(-1)                # (B, wordD)
